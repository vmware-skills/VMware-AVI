...
Every CLI command that reaches the AVI Controller now passes the same `guard()` as its MCP twin and writes one
audit row under that tool's name (23 commands, `@audited`). Commands that reach nothing remote say so, with a reason (2, `@cli_local`).
...
**`helm` and `kubectl` output is decoded as UTF-8.** Eight `subprocess` calls used
`text=True` without an encoding, so they decoded with the machine's locale codec
-- a chart name or an error message with a non-ASCII byte raised
`UnicodeDecodeError` from inside the AKO tooling on a cp936 host.
...
Also in this release: `ako_config_diff` and `ako_config_upgrade` return raw
`helm diff`/`upgrade` output, which renders `avicredentials.password` and the
`avi-secret` Secret — and the audit database stored return values verbatim. Both
now declare `sensitive_result`. Policy credential-key net cannot help here: that
output is one free-form string, not a keyed field, so the declaration is the
only thing keeping it out of the row.
...
- **server.json advertised a stale tool count.** That number is what MCP Registry
  ...
  tools: 29 advertised, 28 real. README and SKILL.md were already right.

Also new: this repo is installable as a Claude Code plugin
(`/plugin install vmware-avi@vmware-skills`). The skill and its MCP server arrive in
...
## v1.8.9 — moved to vmware-skills org + MCP Registry namespace io.github.vmware-skills/vmware-avi
